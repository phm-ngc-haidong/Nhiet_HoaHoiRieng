/* ============================================================
   THÍ NGHIỆM: NHIỆT HÓA HƠI RIÊNG CỦA NƯỚC
   L = P·t·(1/Δm)      (Δm: khối lượng nước hóa hơi)
   Đồ thị: trục hoành 1/Δm, trục tung L
   ============================================================ */
/* Mục 2 — nhập tay: P, Δt, m1 (khối lượng nước đầu), m2 (khối lượng nước còn lại).
   Δm được TỰ TÍNH từ m1, m2 (lấy giá trị tuyệt đối của m1 − m2 nên nhập theo chiều nào cũng ra số dương). */
const FIELDS = ['P', 't', 'm1', 'm2'];
/* Dữ liệu mẫu minh họa — nước hóa hơi ở công suất không đổi P = 1000 W */
const SAMPLE = [
  { P: 1000, t: 60, m1: 0.34780, m2: 0.32125 },
  { P: 1000, t: 60, m1: 0.32850, m2: 0.30195 },
  { P: 1000, t: 60, m1: 0.30730, m2: 0.28075 },
  { P: 1000, t: 60, m1: 0.29761, m2: 0.27106 },
  { P: 1000, t: 60, m1: 0.26407, m2: 0.23752 },
];

/* Mục 1 — số liệu THẬT của nhóm, cố định, không sửa được trên giao diện.
   Nguồn: bảng ghi tay của nhóm (P = 500 W, Δt = 3,0 s, 5 lần đo).
   L trung bình ≈ 2 316 634,346 J/kg (khớp đúng bảng ghi tay). */
const FIXED_DATA = [
  { P: 500, t: 30, m1: 3.2324, m2: 3.2256, dm: 0.0068 },
  { P: 500, t: 30, m1: 3.2256, m2: 3.2193, dm: 0.0063 },
  { P: 500, t: 30, m1: 3.2193, m2: 3.2128, dm: 0.0065 },
  { P: 500, t: 30, m1: 3.2128, m2: 3.2065, dm: 0.0063 },
  { P: 500, t: 30, m1: 3.2065, m2: 3.2000, dm: 0.0065 },
];

const RESULT_LABEL = 'L';
const RESULT_UNIT = 'J/kg';
const Y_AXIS_UNIT = '·10⁶ J/kg'; // đơn vị hiển thị trên trục tung (giá trị tick = J/kg ÷ 10⁶)
const Y_AXIS_DIV = 1e6;
const X_LABEL = '1/Δm (kg⁻¹)';
const REF_VALUE = 2260000; // giá trị chuẩn của nhiệt hóa hơi riêng nước (J/kg)
/* Trục L: gốc luôn là 0. Vạch chia (đơn vị) = Y_GRID_STEP; độ giãn dọc tính bằng số px cho mỗi 10⁶ J/kg,
...
const Y_GRID_STEP = 1000000;        // vạch chia trục L: 0,2·10⁶ J/kg
...
const sections = {
  fixed: {
    ...
    exactResult: true,
    editable: false,
    tableId: 'vapor-table-fixed',
    summaryId: 'vapor-summary-fixed',
    canvasId: 'vapor-chart-fixed',
    calcId: 'calc-fixed'
  },
  editable: {
    rows: [],
    ...
    editable: true,
    tableId: 'vapor-table',
    summaryId: 'vapor-summary',
    canvasId: 'vapor-chart',
...
/* Hiển thị KHÔNG làm tròn phần nguyên: 2 300 000,1234 */
...
/* Số nguyên có dấu cách nghìn (2 260 000) */
function fmtInt(v) {
...
/* Như fmtExact nhưng bỏ các số 0 thừa ở cuối phần thập phân: 2 316 634,3460 → 2 316 634,346 */
...
/* Mục 2: Δm = |m1 − m2| (làm tròn 10 chữ số để bỏ sai số dấu phẩy động) */
function syncDm(row) {
  row.dm = (isFiniteNum(row.m1) && isFiniteNum(row.m2))
    ? Math.round(Math.abs(row.m1 - row.m2) * 1e10) / 1e10
    : NaN;
}
...
function updateSummaryAndChart(section) {
  ...
  /* Sai số tương đối của L trung bình so với giá trị chuẩn 2 260 000 J/kg */
  ...
  document.getElementById(section.summaryId).innerHTML = `
    ...
    <div class="stat">
      <span class="label">Nhiệt hóa hơi riêng trung bình</span>
...
function sumExpr(items) {
  const term = it => `<i>L</i><sub>${it.idx}</sub>`;
  if (!items.length) return '<i>L</i><sub>1</sub> + <i>L</i><sub>2</sub> + … + <i>L</i><sub><i>n</i></sub>';
...
function renderCalc(section, avg, relErr) {
  ...
  const rows = items.map(it => `
    <div class="calc-line math">
      ...
      <span><i>L</i><sub>${it.idx}</sub> =</span>
  ...
  el.innerHTML = `
    <div class="calc-block">
      <h4>Nhiệt hóa hơi riêng của từng lần đo</h4>
    ...
    <div class="calc-block">
      <h4>Nhiệt hóa hơi riêng trung bình</h4>
      <div class="calc-line math">
        <span><span class="ov"><i>L</i></span> =</span>
    ...
    <div class="calc-block">
      ...
      <div class="calc-line math">
        <span><i>δL</i> =</span>
        <span class="mfrac">
          <span>|<span class="ov"><i>L</i></span> − <i>L</i><sub>chuẩn</sub>|</span>
          <span><i>L</i><sub>chuẩn</sub></span>
        </span>
...
/* Nhãn số trên trục L (đơn vị ·10⁶ J/kg): số chữ số thập phân tự theo bước lưới (0,1 · 0,05 · …) */
...
function computeAxisScale(points, availPlotW) {
  ...
  /* Trục L: gốc 0, vạch chia cố định, độ giãn dọc cố định (không ép vừa khung) */
...
function drawChart(canvas, state, hover) {
  ...
  /* Trục L luôn bắt đầu từ 0; miền chỉ được NỚI RỘNG thêm khi có điểm nằm ngoài, không co giãn lại */
  const domainMinY = 0;
  ...
  /* Đường chuẩn L = 2 300 000 J/kg (nét đứt nhẹ) để so sai số tương đối */
  {
    ...
    /* Vạch 2,26·10⁶ trên trục L: gạch nhỏ + nhãn đỏ đậm để đọc thẳng giá trị chuẩn */
